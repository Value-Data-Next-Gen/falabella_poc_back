...
def get_state(user: CurrentUser = Depends(current_user)):
    ...
    # Si hay un plan real cargado para STATE.today (en cualquier estado:
    # BORRADOR/VALIDADO/EN_CURSO/CERRADO), los vehicle_id que reporta
    # /api/state deben ser los del plan REAL (patente_falsa), NO los sintéticos
    # legacy [1..12] del simulador. Sin esto, MapaTab filtra contra IDs que no
    # existen en /api/plan-diario y oculta pines.
    real_vehicles: list[int] = []
    try:
        ...
        with get_conn() as cn:
            ...
            real_vehicles = sorted(int(r.patente_falsa) for r in cur.fetchall())
    except Exception:  # noqa: BLE001
        real_vehicles = []
    vehicles = real_vehicles or sorted(int(v) for v in df["vehicle_id"].unique())
...
def get_visits(
    ...
    if only_alerts:
        if eta_window_hours is not None or alert_threshold is not None:
            from ml.pipeline import compute_alert_mask

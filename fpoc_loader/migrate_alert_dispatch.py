...
Agrega a `fpoc.empresas_transporte`:
  - supervisor_phone_e164  NVARCHAR(20) NULL

Crea `fpoc.alert_dispatch_log` (idempotente):
  - alert_id           PK
  - tracking_id        NVARCHAR NOT NULL (indexado)
  - type               'retraso_vip' | 'driver_sin_respuesta' | 'motivo_patron'
  - channel            'whatsapp' | 'sms' | 'in_app'
  - target             'cliente' | 'driver' | 'supervisor'
  - sent_at            DATETIME2
  - acknowledged_at    DATETIME2 NULL
  - ruta_id            NVARCHAR NULL
  - empresa_id         INT NULL
  - payload_json       NVARCHAR(MAX) NULL (rendered template, debug)
  - created_by_user_id INT NULL

Azure SQL único backend.
"""
...
def _add_column(cn, table: str, column: str, sqlserver_ddl: str) -> bool:
...
def _create_alert_dispatch_log(cn) -> None:
    if _table_exists(cn, "fpoc_alert_dispatch_log"):
        print("[skip] fpoc.alert_dispatch_log ya existe")
        return
    cur = cn.cursor()
    cur.execute("""
    ...
    cn.commit()
    print("[ok]   fpoc.alert_dispatch_log creada con 3 índices")

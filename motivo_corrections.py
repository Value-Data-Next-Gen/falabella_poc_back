...
from motivo_classifier import _classify_llm
...
def maybe_create_correction_from_comment(
    *,
    ...
    if os.environ.get("ENABLE_AUTO_NOTIFY", "false").lower() == "true":
        ...
        try:
            from comments import _resolve_alert_targets, _backfill_contact_id_in_log
            from notifications import send_whatsapp
...
def accept_correction(correction_id: int, user: CurrentUser = Depends(current_user)) -> MotivoCorrectionOut:
    ...
    try:
        from comments import _resolve_alert_config, dispatch_comment_alert, severity_rank
